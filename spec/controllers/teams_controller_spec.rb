...
RSpec.describe TeamsController do
  ...
  describe "GET show" do
    ...
    it 'lists the team conference preferences' do
      get :show, params: { id: team.id }
      expect(response).to be_success
      expect(response.body).to match preference.first_choice.name
      expect(response.body).to match preference.second_choice.name
    end
  end
        ...
        context 'selecting the conference options' do
          ...
          let(:team_params) do
            build(:team).attributes.merge(:conference_preference_attributes=>{
              first_choice_id: conference_1.id,
              second_choice_id: conference_2.id
            })
          end
          ...
          it 'student can set one conference preference with two conference options for her team' do
            ...
            expect(team.conference_preference.first_choice).to eql conference_1
            expect(team.conference_preference.second_choice).to eql conference_2
          end
        end
      end
